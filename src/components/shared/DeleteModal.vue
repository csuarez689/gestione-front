<template>
	<b-modal size="sm" centered no-close-on-backdrop no-close-on-esc ref="modal">
		<template v-slot:modal-header>
			<h5 class>Eliminar {{ modelName | capitalize }}</h5>
		</template>
		<template v-slot:default>
			<p class="text-center">¿Está seguro que desea borrar el {{ modelName }}?</p>
			<p class="text-danger text-center">
				<small>Los cambios son permanentes</small>
			</p>
		</template>
		<template v-slot:modal-footer="{ cancel }">
			<b-button size="md" variant="outline" @click="cancel()">Cancelar</b-button>
			<b-button size="md" variant="danger" @click="deleteItem()">Eliminar</b-button>
		</template>
	</b-modal>
</template>
<script>
